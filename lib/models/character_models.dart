...
class Wand {
    ...
    factory Wand.fromJson(Map<String, dynamic> json) => Wand(
        wood: woodValues.map![json["wood"]],///CAMBIO ?
        core: coreValues.map![json["core"]],///CAMBIO ?
        length: json["length"],
    );
...
class EnumValues<T> {
    Map<String, T>? map;  ///CAMBIO ?
    Map<T, String>?reverseMap; ///CAMBIO ?

    EnumValues(this.map);

    Map<T, String> get reverse {
        if (reverseMap == null) {
            reverseMap = map!.map((k, v) => new MapEntry(v, k)); ///CAMBIO ?
        }
        return reverseMap!; ///CAMBIO ?
    }
}
